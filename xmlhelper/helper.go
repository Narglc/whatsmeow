...
func main() {
	// testToken := []string{"iq", "id", "to", "g.us", "type", "set", "xmlns", "w:g2", "create", "key", "subject",
	// 	"participant", "jid", "s.whatsapp.net", "-", "80", "217"}
	testToken := []string{"notification", "addressing_mode", "pn", "g.us", "from", "id", "notify", "w:g2", "participant", "t", "type",
		"member_add_mode", "all_member_add", "s.whatsapp.net", "w:gp2"}

	for _, data := range testToken {
		var dictIndex byte
		if tokenIndex, ok := IndexOfSingleToken(data); ok {
			fmt.Printf("token:%s, type:single index:%X\n", data, tokenIndex)
		} else if dictIndex, tokenIndex, ok = IndexOfDoubleByteToken(data); ok {
			fmt.Printf("token:%s, type:double dictNo:%X, index:%X\n", data, Dictionary0+dictIndex, tokenIndex)
		} else {
			fmt.Printf("token:%s not found.\n", data)
		}
	}

	originText := []string{"120363216419776256", "1787767991", "晓宇飞飞飞", "8613714726970", "1707198298", "all_member_add", "admin_add", "addressing_mode"}
	for _, one := range originText {
		fmt.Printf("%s--->%X\n", one, []byte(one))
	}
